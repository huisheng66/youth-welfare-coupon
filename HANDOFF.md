...
更新时间：2026-09-06（第十四批：T23 发布验收包 v1.5.0）

## 已完成

`log.md` 已记录并通过对应回归测试的修改。截至 2026-09-06 第十四批，
`docs/开发计划.md` 检查表 T00–T23 全部收口（T21 真机项与 T23 staging 实机
验收、发布后观察窗口属运维动作）。发布包见 `docs/release-v1.5.0.md`：
版本 1.5.0、迁移 head `d8f2a06c4b11`、升级前置/回退路径/观察窗口/角色验收清单。
...
37. T23 发布验收包与文档交接：版本 1.5.0；docs/release-v1.5.0.md（变更清单、
    升级前置、部署步骤、回退路径、风险与观察窗口、角色验收清单）；README/
    PRODUCT/deploy-README 过时说法清理（建表说法、统一初始密码、已预留等）。
...
1. staging/生产实机走 `docs/release-v1.5.0.md` 第 6 节角色验收清单后正式发布；
   发布后按第 5 节观察窗口跟踪 7 天。
2. 历史泄漏凭据的轮换证据收集、生产服务器实际状态核实、季度恢复演练
   （首次已演练，见 log.md 2026-09-05）。
3. T21 真机验收（真实手机摄像头 + HTTPS 信任）随发布一并完成。
